...
function update_car_location_options($term_id = '') {
	...
	if ( isset( $_POST['location_name'] ) ) {
		...
		$current_location = sanitize_text_field( $_POST['location_name'] );
	...
	if ( isset( $_POST['tag-name'] ) ) {
		$current_location = sanitize_text_field( $_POST['tag-name'] );
		$current_location = trim( $current_location );
		$current_location = strtolower( $current_location );
		$current_location = str_replace( ' ', '-', $current_location );
		$location_slug = 'default';
	...
	if ( isset( $_POST['slug'] ) ) {
		$current_location = sanitize_text_field( $_POST['slug'] );
		$location_slug = sanitize_text_field( $_POST['slug'] );
	}
	...
	foreach ( $data as $key=>$value ) {
		...
		$location_data[ $current_location ][ $key ] = sanitize_text_field( $value );
	}

	// begin storing all location information in a single array
	update_option( 'location_'.$current_location, $location_data, 'no' );
	...
	foreach( $fields as $field=>$data ) {
		if ( isset( $_POST[ $current_location . $data['field'] ] ) ) { 
			$value = sanitize_text_field( $_POST[ $current_location . $data['field'] ] );
			$value = str_replace( "\'", "'", $value );
			$value = str_replace( '\"', '"', $value );
			$value = str_replace( '\\', '', $value );
			$value = apply_filters( $data['field'] . '_location_filter', $value, $current_location );
			update_option( $current_location . $data['field'], $value, 'no' );
		}
	}

	$car_demon_settings_hook = apply_filters( 'car_demon_admin_update_hook', $fields, $location_slug ); //= deprecated
...
if (defined('CD_LOCATIONS')) {
	...
	function add_car_location_options( $term_id = '' ) {
		...
		$fields = cd_get_location_fields();
		foreach( $fields as $field=>$data ) {
			if ( isset( $_POST[ $location_slug . $data['field'] ] ) ) { 
				$value = sanitize_text_field( $_POST[ $location_slug . $data['field'] ] );
				$value = str_replace( "\'", "'", $value );
				$value = str_replace( '\"', '"', $value );
				$value = str_replace( '\\', '', $value );
				$value = apply_filters( $data['field'] . '_location_filter', $value, $current_location );
		...
		$car_demon_settings_hook = apply_filters( 'car_demon_admin_update_hook', $fields, $location_slug );
		do_action( 'cd_location_update_action', $location_slug );
	}
	...
	function cd_get_location_fields() {
		...
		//= Create empty array
		$valid_fields = array();
		//= Loop array and if an item doesn't already exists then add to our new array
		foreach( $fields as $field ) {
			//= Check if item with the same name already exists
			if ( ! cd_in_array_r( $field['field'], $valid_fields ) ) {
				$valid_fields[] = $field;
			}
		}
	...
	function cd_in_array_r( $needle, $haystack, $strict = false ) {
		foreach ( $haystack as $item ) {
			if ( ( $strict ? $item === $needle : $item == $needle ) || ( is_array( $item ) && cd_in_array_r( $needle, $item, $strict ) ) ) {
				return true;
			}
		}
		return false;
