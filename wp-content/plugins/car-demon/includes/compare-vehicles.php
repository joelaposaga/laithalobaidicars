...
function show_compare_vehicles() {
	...
	if ( $compare_these_array ) {
		$compare_these = explode( ',', $compare_these_array );
			...
			foreach ( $compare_these as $post_id ) {
				...
				$compare_item = '<input checked="checked" type="checkbox" onclick="update_car(' . $post_id . ',this);" data-post-id="' . $post_id . '" />&nbsp;';
	...
	$x = '<p>' . $x . '</p>';
	$x = apply_filters( 'cd_compare_widget_items_filter', $x, $_SESSION['car_demon_compare'] );
	return $x;
}
function show_compare_list() {
	...
	$x = apply_filters( 'cd_compare_all_filter', $x, $_SESSION['car_demon_compare'] );
	return $x;
}
?>