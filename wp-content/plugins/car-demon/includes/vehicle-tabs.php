...
function get_option_tab( $tab, $post_id, $type='' ) {
	...
	if ( isset( $map[$tab] ) ) {
		foreach( $map[$tab] as $tab_group => $value ) {
			...
			//= create an array of slugs to compare to _vehicle_options below
			$slugs = array();
			if ( ! empty( $value ) ) {
				...
				//= TO DO - Make sure duplicates do not show
				//= tied to vin-query-handler.php line 165
