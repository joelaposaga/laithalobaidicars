...
function cd_inventory_shortcode_func( $atts ) {
	...
	$default_atts = array(
		...
		__( 'show_only_sold', 'car-demon' ) => '', /* Show only sold vehicles in list */
	...
	$atts = shortcode_atts( $default_atts, $atts, 'cd_inventory' );
...
function cd_show_inventory( $atts ) {
	...
	if ( isset( $atts['query'] ) ) {
		if ( ! empty( $atts['query'] ) ) {
			...
			foreach ( $query_parts as $query_part ) {
				...
				if ( ! empty( $part[1] ) ) {
					if ( trim( $part[0] ) != 'stock' && trim( $part[0] ) != 'mileage' && trim( $part[0] ) != 'min_price'  && trim( $part[0] ) != 'max_price'  && trim( $part[0] ) != 'show_sold' && trim( $part[0] ) != 'show_only_sold' && trim( $part[0] ) != 'criteria' && trim( $part[0] ) != 'cars_per_page' ) {
	...
	if ( isset( $query['show_only_sold'] ) ) {
		if ( ucwords( $query['show_only_sold'] ) === 'Yes' ) {
			$car_demon_query['meta_query'] = array(
				'key' => __( 'sold', 'car-demon' ),
	...
	ob_end_clean();
		...
		ob_start();
		do_action( 'cd_after_sort_srp_action', $atts );
		$html .= ob_get_contents();
		ob_end_clean();
		...
		ob_start();
		do_action( 'cd_after_results_found_srp_action', $atts );
		$html .= ob_get_contents();
		ob_end_clean();
		...
		ob_start();
		do_action( 'cd_after_nav_srp_action', $atts );
		$html .= ob_get_contents();
		ob_end_clean();
		...
		if ( $car_query->have_posts() ) : while ( $car_query->have_posts() ) : $car_query->the_post();
			...
			if ( function_exists( 'cdcr_loop' ) ) {
				$loop = '<div class="car_item">' . cdcr_loop( '', $post_id, $atts ) . '</div>';
			} else {
			...
			$html .= apply_filters( 'cd_srp_filter', $loop, $post_id, $atts );
